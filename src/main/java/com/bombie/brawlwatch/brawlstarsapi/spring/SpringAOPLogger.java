...
public class SpringAOPLogger {
    @Pointcut("execution(* *(..)) && within(com.bombie.brawlwatch.brawlstarsapi.util..*)")
    public void methodCallInServiceLayer() {
    }

    @Pointcut("@annotation(org.springframework.context.annotation.Bean) && within(com.bombie.brawlwatch.brawlstarsapi..*)")
    void beanMethodCall() {
    }
    ...
    public Object logMethodExecutionTime(ProceedingJoinPoint jp) throws Throwable {
        ...
        try {
            result = jp.proceed();
            stopWatch.stop();
            logger.info("returned: " + " after " + stopWatch.getTotalTimeMillis() + "ms: " + methodName + "(" + toPrintableArray(args) + ") => "
                    + toPrintable(result));
        } catch (Throwable e) {
